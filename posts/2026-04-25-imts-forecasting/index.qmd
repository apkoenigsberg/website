...
date: "2026-04-25"
categories: [trade, forecasting, anomaly-detection, time-series]
image: "preview.png"
format:
  html:
    code-fold: true
...
plt.savefig(_DIR / 'fig_stacked_shares.png', bbox_inches='tight', dpi=150)
plt.show()
```
...
We need 12 months of history before making any prediction, so the first valid residual is at month 13 of each series. The `min_periods=12` argument enforces this — we don't allow partial windows.
...
for country in TARGET:
    ...
    metrics_rows.append({
        ...
        'Fit quality':   '★★★★★' if _safe_mean(rmses)*100 < 1 else
                         '★★★★☆' if _safe_mean(rmses)*100 < 2 else
                         '★★★☆☆' if _safe_mean(rmses)*100 < 5 else
                         '★★☆☆☆' if _safe_mean(rmses)*100 < 10 else '★☆☆☆☆'
    })
...
plot_anomaly_country('UKR', save_path=_DIR / 'fig_anomaly_ukr.png')
```
...
plot_anomaly_country('VEN', save_path=_DIR / 'fig_anomaly_ven.png')
```
...
plot_anomaly_country('YEM', save_path=_DIR / 'fig_anomaly_yem.png')
```
...
plot_anomaly_country('MEX', save_path=_DIR / 'fig_anomaly_mex.png')
```
...
plot_anomaly_country('DNK', save_path=_DIR / 'fig_anomaly_dnk.png')

# Save preview
import shutil
shutil.copy(_DIR / 'fig_anomaly_ukr.png', _DIR / 'preview.png')
```
...
*Data: [IMF International Merchandise Trade Statistics](https://data.imf.org/IMTS), bulk flat file download. All code in this post runs without `scipy` or `pycountry` — only `pandas`, `numpy`, `matplotlib`, and Python's standard `math` module.*
